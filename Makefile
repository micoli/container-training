...
02-kubectl-queries-01-apply-example:
	- kubectl delete -f 01-Simple/
	kubectl apply -f 01-Simple/

02-kubectl-queries-02-get-all-pods:
	kubectl get pods

02-kubectl-queries-03-get-all-pods-all-namespace:
	kubectl get pods --all-namespaces

02-kubectl-queries-04-get-all-object:
	kubectl get all

02-kubectl-queries-05-get-pods-name:
	kubectl get pods -o name

02-kubectl-queries-06-get-pods-info-widely:
	kubectl get pods -o wide

02-kubectl-queries-07-execute-one-command-in-a-pod:
	kubectl exec nginx cat /etc/nginx/conf.d/default.conf

02-kubectl-queries-08-view-pod-metadata:
	kubectl describe pod nginx

02-kubectl-queries-09-view-pods-as-json:
	kubectl get pods --field-selector=status.phase=Running -o=json | jq

02-kubectl-queries-10-view-pods-and-format:
	kubectl get pods --field-selector=status.phase=Running -o=jsonpath='{.items[*].metadata.name}'

02-kubectl-queries-11-view-pods-as-json:
	kubectl get pods --field-selector=status.phase=Running -o=json | jq ".items[0].spec.containers"

02-kubectl-queries-12-view-pods-log:
	kubectl logs nginx -c nginx

02-kubectl-queries-13-execute-simple-http-call:
	kubectl port-forward pod/nginx 8080:80 &;kubectlPid=$!
	curl http://127.0.0.1:8080
	kill $kubectlPid

02-kubectl-queries-99-cleanup:
	- kubectl delete -f 01-Simple/
	kubectl get all


03-kubernetes-selector-01-:
	- kubectl delete -f 02-Selector/
	kubectl apply -f 02-Selector/

03-kubernetes-selector-02-label-queries:
...
03-kubernetes-selector-03-extended-label-queries:
...
03-kubernetes-selector-04-selector-for-delete:
	kubectl delete pods -l service=cdn
	kubectl get pods

03-kubernetes-selector-99-cleanup:
	- kubectl delete -f 02-Selector/
	kubectl get all


04-kubernetes-load-balancing-01-build-image:
	03-Loadbalancing/container/build.sh

04-kubernetes-load-balancing-02-apply-example:
	- kubectl delete -f 03-Loadbalancing/
	kubectl apply -f 03-Loadbalancing/
	sleep 10

04-kubernetes-load-balancing-03-access-service:
	http http://localhost:8001/api/v1/namespaces/default/services/http:frontend-service:web/proxy/

04-kubernetes-load-balancing-04-test-load-balancing:
...
04-kubernetes-load-balancing-99-cleanup:
	- kubectl delete -f 03-Loadbalancing/
	kubectl get all
